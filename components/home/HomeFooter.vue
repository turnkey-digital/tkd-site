...
<div>
    ...
    <div class="bg-pri py-24 text-2xl leading-10">
        <div class="container px-8 mx-auto text-center">
            <h3 class="text-4xl text-black text-left font-black font-helN">{{header}}</h3>
            <h4 class="text-xl text-black text-left font-black font-helN">{{tagLine}}</h4>
            <NuxtLink to="contact" class="mx-auto border  px-4 py-2 mt-8 leading-none text-base text-black font-helN">{{btnTxt}}</NuxtLink>
        </div>
    </div>
    <div>
        <div v-bind:class="{ 'is-show': isActive }" class="footerModal block h-screen w-screen bg-black">
            <div class="container mx-auto py-24"  v-html="copyRight" />
        </div>
